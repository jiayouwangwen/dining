...
public class Control {
	...
	private UserInput ui;
     ...
     private final String Ip="10.10.49.152";
    ...
    loopss:	 if(n==1){
    			 while(true){
    				 ...
    				 String password = this.ui.getString("请输入员工密码:");
    				  employee str = this.service.logEm(eid, password);
    				 if(str!=null){
    					 break;
    				 }
    	    ...
    	    loops:			if(m==0){
    			 ...
    			 while(true){
    				 ...
    				 manager str = this.service.logM(eid, password);
    				 if(str!=null){
    					 break;
    				 }
    				 this.v.println("id或密码错误，重新输入");
	...
	//删除员工信息
     private String  deleteEmployee() {
		...
		employee emp = this.service.findEmp(empid);
		if(emp==null){
			return "员工不存在";
		}
	...
	//修改员工信息
     private String  alterEmployee() {
		int id = this.ui.getInt("输入要修改的员工的id编号");
		  employee emp = this.service.findEmp(id);
		if(emp==null){
			return "员工不存在";
		}else if(emp!=null){
	...
	//经理对普通员工的添加
     private String  addEmployee() {
		...
		return this.service.insertEmployee(new employee(empid, name, password));
	}
     //打印小票
	...
	private String  addCard() {
		...
		int id = this.ui.getInt("输入客户id：");
		//调用service方法查看客户是否存在
		 custumer cus = this.service.findCustumerByE(id);
		if(cus!=null){
			return "该用户已存在";
		}
